use biome_tailwind_sorter::{sort_tailwind_classes, TailwindFormatter};
...
fn generate_test_content(num_components: usize, classes_per_component: usize) -> String {
    ...
    for i in 0..num_components {
        content.push_str(r#"<div className=""#);
...
fn bench_sort_classes(c: &mut Criterion) {
    ...
    let large_classes: Vec<String> = (0..100)
        .map(|i| {
            let class_pool = [
                "text-red-500",
                "p-4",
...
fn bench_regex_compilation(c: &mut Criterion) {
    ...
    static STATIC_REGEX: LazyLock<Regex> =
    ...
    group.bench_function("static_compilation", |b| {
        b.iter(|| STATIC_REGEX.find_iter(black_box(&test_content)).count())
    });
