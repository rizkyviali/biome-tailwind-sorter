...
class TailwindSorter {
    ...
    calculateOffset(document, position) {
        let offset = 0;
    ...
    async sortTailwindClasses(document, preserveCursor = true, cursorPosition) {
...
function activate(context) {
    ...
    const formatOnSaveDisposable = vscode.workspace.onWillSaveTextDocument(async (event) => {
        const config = vscode.workspace.getConfiguration('biome-tailwind-sorter');
        if (!config.get('enable', true) || !config.get('formatOnSave', false)) {
            return;
        }
        ...
        try {
            ...
            event.waitUntil(vscode.window.activeTextEditor?.edit(editBuilder => {
                const fullRange = new vscode.Range(event.document.positionAt(0), event.document.positionAt(event.document.getText().length));
                editBuilder.replace(fullRange, result.content);
