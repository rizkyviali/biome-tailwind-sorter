...
fn test_cli_check_mode_already_formatted() {
    ...
    assert_eq!(
        output.status.code().unwrap(),
        0,
        "Should exit with code 0 when already formatted"
    );
}
...
fn test_cli_write_mode_formats_file() {
    ...
    assert_eq!(
        output.status.code().unwrap(),
        0,
        "Should exit with code 0 on successful write"
    );

    let formatted_content = fs::read_to_string(&file_path).expect("Failed to read formatted file");
    assert!(
        formatted_content.contains(r#"class="flex p-4 text-red-500""#),
...
fn test_cli_cursor_preservation() {
    ...
    assert!(
        stderr.contains("CURSOR_POSITION:"),
...
fn test_cli_verbose_output() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(
        stdout.contains("Formatted") || stdout.contains("formatted"),
...
fn test_cli_multiple_files() {
    ...
    fs::write(
        &file2_path,
        r#"<div class="text-blue-500 m-2 inline">test2</div>"#,
    )
...
fn test_cli_directory_processing() {
    ...
    fs::write(
        &file1_path,
        r#"<div className="text-red-500 p-4 flex">test</div>"#,
    )
    ...
    fs::write(
        &file2_path,
        r#"<div class="text-blue-500 m-2 inline">test</div>"#,
    )
...
fn test_cli_unsupported_files_ignored() {
    ...
    assert_ne!(
        output.status.code().unwrap(),
        0,
        "Should exit with error when no supported files"
    );
...
>
  Button
    ...
    assert!(
        p4_pos < font_pos && font_pos < bg_pos,
